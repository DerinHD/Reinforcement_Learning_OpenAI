...
class BaseEnvironment(ABC, gym.Env):
    ...
    def __init__(self): 
        ...
        action_space (any):
            Action space (e.g., gym.spaces.Discrete)

        observation_space (any):
            Observation space (e.g., gym.spaces.Discrete)
    ...
    def set_reward_function(self, reward_Function:str):
        ...
        reward_function_type (str): Type of reward function
            "Default": Use standard reward function reeceived from the environment
            "Approx_IRL": Use reconstructed reward function generated by the approx. IRL algorithm
        """
        rewardFunctionsPossible = ["Default", "Approx_IRL"]
    ...
    def env_step(self, action):
        """
        Perform an action in the environment. This function is used to perform the step in the environment. 
    ...
    def step(self, action):
        """
        Perform an action in the environment. It calls the method env_step() that needs to be implemented in the inherited class.
        ...
        next_state, reward , termination, truncation, info = self.env_step(action) # perform step 

        rewardUsed = reward

        if self.reward_Function == "Approx_IRL":
            feature= self.transform_state_action_to_feature(self.current_state, action)
            rewardUsed = feature @ self.theta
    ...
    def render(self, mode = "human"):
        """
        Renders the environment. This function is used to render the environment in a human-readable format.

        Args:

        mode (str):
            Mode of rendering (human, rgb_array, etc.)
        """
        raise NotImplementedError
    
    def approx_IRL_configure(self, theta: numpy.ndarray):
        """
        configure the weight vector of the approx. IRL algorithm.
